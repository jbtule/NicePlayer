...
@implementation JTPublicCryptKey
...
+(id)keyWithData:(NSData*)aData withAlgortihm:(uint32)anAlgorithm{
    ...
    hdr->KeyClass = CSSM_KEYCLASS_PUBLIC_KEY;//changes per class
          
        
        hdr->KeyAttr = CSSM_KEYATTR_EXTRACTABLE;
        hdr->KeyUsage = CSSM_KEYUSE_ENCRYPT | CSSM_KEYUSE_VERIFY;
...
-(BOOL)verifyData:(NSData*)aData withSignature:(NSData*)aSig{
    ...
    sig.Data = (uint8*)[aSig bytes];
    sig.Length = [aSig length];
    
    
    sigAlg = [self signitureAlgorithm];
    
    if(!sigAlg){
        NSLog(@"Unsupported Algorithm for verifying");
    }
    
    crtn = cdsaVerify(_cspHandle,
                    &_key,
                    sigAlg,
                    &ptext,
                    &sig);
